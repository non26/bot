...
func (s *trailingStopBarService) ByTrailingStopBarShort(ctx context.Context,
	request *domain.TrailingStopBarDomain,
	tradeRequest *domain.Trade,
	botOpening *domain.BotDomain) string {

	tradeRequest.PositionSide = bnconstant.SHORT
	msg := CONTINUE
	...
	if botOpening == nil {
		...
		if !trailingStopBarRestriction.IsTargetPositionShortSide() {
			return msg
		}

		if request.Candle0.IsRedCandle() {
			...
			if trailingStopBarRestriction.CurrentPriceMorethanLastBarOpenPrice(request.Candle0.Close) {
